...
class WebDataset(IterableDataset):
    ...
    def __init__(
        ...
        if target_transform is not None:
            ...
            self.target_transform = lambda x: x + label_offset

        self.dataset = wds.WebDataset(
            glob(f"{root}/{base_fname}/*.tar"), shardshuffle=shuffle
        )
        if shuffle:
            self.dataset = self.dataset.shuffle(1000)
        self.data = (
            self.dataset.decode("pil")
    ...
    def __iter__(self):
        ...
        if self.interleave:
            ...
            if self.batch_size is None:
                ...
                sources = [
                    iter(ds.batched(batch_size))
                    for ds, batch_size in zip(self.datasets, self.batch_sizes)
                ]
